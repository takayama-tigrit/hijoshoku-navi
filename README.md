...
写真・図解・ニーズ階層・その場の行動に関する受入条件は [`docs/editorial-contract.md`](docs/editorial-contract.md) を正本とします。写真の利用条件は `docs/image-licenses.json`、図の数値は `data/quantities.json` で管理します。

備蓄計算はブラウザ内だけで実行し、入力値を送信・永続保存しません。保存ボタンは端末へのテキストダウンロードです。検証では計算だけでなく保存内容、不正入力、既存在庫が十分な場合も確認します。
