...
const routes={'/ranking/':3,'/guide/':0,'/posts/alpha-mai-osusume/':3,'/posts/emergency-food-set-check/':2,'/posts/emergency-food-side-dishes/':2};
...
const groups={'/ranking/':'ranking','/posts/alpha-mai-osusume/':'alpha','/posts/emergency-food-set-check/':'sets','/posts/emergency-food-side-dishes/':'sides'};
...
try{
 for(const device of [{name:'mobile',...devices['iPhone 13']},{name:'narrow',viewport:{width:320,height:740}},{name:'desktop',viewport:{width:1440,height:1000}},{name:'noJS',...devices['iPhone 13'],javaScriptEnabled:false}]){
  ...
  for(const [route,count]of Object.entries(routes)){
   ...
   check(!!data.og&&data.og.startsWith('https://hijoshoku-navi.com/'),`${name} ${route}: owned OG image`);
   ...
   check(article?.author?.name==='非常食ナビ編集部'&&!!article?.dateModified&&!!article?.image,`${name} ${route}: truthful author/date/image`);
