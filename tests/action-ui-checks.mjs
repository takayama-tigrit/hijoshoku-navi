...
export async function checkVisualAction(page, route, width) {
  ...
  // v7: chart counts are not value. Only preparation and salt comparisons remain.
  const minimumCharts = { '/guide/': 1, '/ranking/': 1, '/posts/alpha-mai-osusume/': 2 };
  ...
  if (['/', '/guide/'].includes(route)) {
    ...
    await planner.locator('[name="litres"]').fill('200');
    await planner.locator('[data-calculate]').click();
