...
for(const [name,text] of Object.entries({guide,ranking,alpha})) {
  for(const token of ['主菜','副菜','アレルギー','15℃','60分','生活用水']) assert(text.includes(token),`${name}: missing condition ${token}`);
  assert(/残存賞味期限|届く時点で賞味期限がどれだけ残っているか/.test(text), `${name}: remaining shelf-life condition`);
  assert(!/保証しません|保証できません|判断できません/.test(text),`${name}: defensive copy returned`);
}
assert(/延べ食事回数/.test(guide)&&/食品の袋数とは異なります/.test(guide),'meal occasions remain separate from packages');
assert(/主食だけの合計/.test(guide)&&/1日に必要な熱量を示す値ではありません/.test(guide),'energy sum is not nutritional sufficiency');
...
assert(/乾燥状態の1袋100g当たり/.test(alpha)&&/出来上がり100gの値ではありません/.test(alpha),'nutrition denominator retained');
assert(/1人につき1日3袋を使うと仮定した試算/.test(alpha)&&/飲み水は別/.test(alpha),'water scenario is an assumption, not total water need');
assert(/戻したごはんは、早めに食べ切/.test(alpha)&&/期限内に食べて入れ替え/.test(alpha),'post-preparation and expiry guidance retained');
